...
void main() {
  ...
  group("Happy path", () {
    ...
    test("Body arg can be optional", () async {
      server = await enableController("/", OptionalTestController.new);
      final m = {"name": "Bob"};
    ...
    test("Can read body object declared as property", () async {
      server = await enableController("/", PropertyTestController.new);
      final m = {"name": "Bob"};
      final response = await postJSON(m);
      expect(response.statusCode, 200);
      expect(json.decode(response.body), m);
    ...
    test("Can bind primitive map", () async {
      server = await enableController("/", MapController.new);
      final m = {"name": "Bob"};
      final response = await postJSON(m);
      expect(response.statusCode, 200);
      expect(json.decode(response.body), m);
  ...
  group("Programmer error cases", () {
    test("fromMap throws uncaught error should return a 500", () async {
      server = await enableController("/", CrashController.new);
